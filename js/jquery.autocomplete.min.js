...
(function (g) {
        ...
        this.inputView.attr("autocomplete", "off").keyup(this._keyup = function (s) {
            switch (s.keyCode) {
                ...
                default: if (r.option.async) {
                    setTimeout(function () { h.apply(r) }, 0)
                }
                else {
                    h.apply(r)
                }
    ...
    var j = function () {
        var q = this; this.searchView = g("<div class='ac'><ul></ul></div>").appendTo(document.body).on("mouseenter", "li",
            function () {
        ...
        if (g.inArray(this.option.listStyle, ["normal", "iconList", "custom"]) == -1) {
            ...
            switch (s.option.listStyle) {
                case "normal": v.append("<span>" + w.label + "</span>");
                    break;
                case "iconList": var t = g("<img></img>").attr("src", w.image); v.append(g("<div></div>").append(t)).append("<span>" + w.label + "</span>");
                    break;
    ...
    var k = function () {
        if (this.option.listDirection === "down") {
            ...
            else { throw "遇到未知的listDirection参数！" }
        }
    ...
    var l = function (q) {
        ...
        } this.searchView.show(); p.apply(this, [this.option.listDirection])
    };
    var e = function () {
    ...
    var p = function (s) {
        ...
        if (q.size()) {
            this.searchView.find("li").removeClass("selected"); q.addClass("selected");
            var u = q.outerHeight();
            var r = q.position().top; if (u + r > this.searchView.height()) {
    ...
    var c = function () {
        ...
        if (q.size()) {
            ...
            if (g.isFunction(this.option.afterSelectedHandler)) {
                try { this.option.afterSelectedHandler.apply(r, [s]) }
                catch (t) {
                    ...
                    if (s.option.ajaxDataType === "xml") {
                        g(u).find("item").each(function () {
                            ...
                            for (var y = 0; y < this.attributes.length; y++) {
                                var x = this.attributes[y].nodeName, A = this.attributes[y].nodeValue; z[x] = A
                            } t.push(z)
                        })
                    }
                    else {
                        if (s.option.ajaxDataType === "json") { t = u }
                        else { throw "遇到未知的ajaxDataType参数！" }
                    }
                }, "error": function (u, w, v) { throw v }
            };
        if (g.isPlainObject(this.option.ajaxParams)) {
            r["data"] = g.extend(false, { "keyword": q }, this.option.ajaxParams)
        }
        else {
            if (g.isFunction(this.option.ajaxParams)) {
                r["data"] = g.extend(false, { "keyword": q },
                    this.option.ajaxParams.apply(this, [q]))
            }
            else {
                if (typeof (this.option.ajaxParams) === "string") { r["data"] = "keyword=" + q + "&" + this.option.ajaxParams }
                else {
    ...
    var h = function () {
        ...
        if (g.isFunction(this.option.beforeLoadDataHandler)) {
            try { r = this.option.beforeLoadDataHandler.apply(this, [s]) } catch (v) { o.apply(this, ["调用beforeLoadDataHandler错误:" + v]); return }
        }
        if (r) {
            if (g.isArray(this.option.data)) { u = this.option.data }
            else {
                if (g.isFunction(this.option.data)) {
                    try { u = this.option.data.apply(this, [s]) }
                    catch (v) {
                    ...
                    if (typeof (this.option.data) === "string") {
                        try {
                            ...
                            o.apply(this, ["Ajax错误:" + v]); return
                        }
                    } else {
                        o.apply(this, ["遇到未知的data参数！"]); return
                    }
                }
            }
            ...
            if (g.isPlainObject(y)) {
                ...
                if (typeof (y) === "string") { var x = { "label": y, "value": y, "image": y } }
                else {
                    o.apply(t, ["数据源Item类型错误！"]);
                    return false
                }
            }
            if (t.option.matchHandler.apply(t, [s, x])) { q.push(x) }
        });
    ...
    var o = function (q) {
        if (g.isFunction(this.option.onerror)) { this.option.onerror.apply(this, [q]) }
    };
    f.prototype.setOption = function (q) {
        if (g.isPlainObject(q)) {
            ...
            if (typeof (q) === "string") {
                switch (q) {
                    case "destroy": this.destroy(); break; case "show": this.show(); break; default: o.apply(this, ["未知的AutoComplete参数！"]); return
                }
            } else {
                o.apply(this, ["未知的AutoComplete参数类型！"]); return
            }
        }
    };
    ...
    f.prototype.show = function () {
        if (this.option.async) {
            setTimeout(function () { h.apply(this) }, 0)
        }
        else {
            h.apply(this)
        }
    ...
    var m = function (q, s) {
        var r = RegExp("(" + q.replace(/([.?*+^$[\]\\(){}|-])/g, "\\$1") + ")", "ig"); s.label = s.label.replace(r, "<em>$1</em>")
    }
})(jQuery);